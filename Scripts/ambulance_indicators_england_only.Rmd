...
trends_graph <-  function(data=amb_dta_plot,var.x="North East and Yorkshire"){
  ...
  scale_colour_THF()+
    labs(x = "", y="Response time (hours:minutes)", caption = "NHS England, Ambulance Quality Indicators")+
    ggtitle(var.x)+
...
plot<-amb_response_average_plots %>%
  ...
  facet_grid(cols=vars(met_cat))+
  scale_colour_THF()+
  labs(x = "", y="Response time (hours:mins:secs)", caption = "NHS England, Ambulance Quality Indicators")+
  theme(legend.text=element_text(size=11),
        legend.title = element_blank(),
...
amb_incidents_average_plots<-amb_incidents_average_plots %>% 
  ...
  filter(metric=="all_incidents_rollmean") %>% 
  mutate(met_group= "All incidents") %>% 
  mutate(name=fct_reorder(list_dates,order)) 
...
* Counts of resources assigned to incidents and those that arrived on scene. This includes all trust dispatched resources, private ambulance resources and voluntary ambulance services. 

* Resources can be classed as conveying resource that will transport patients in a time frame appropriate to their clinical needs (usually category 2-4) and/or resources that are appropriate to the patients' needs (if this is on site 'treatment', i.e. trust response vehicle with a health care professional on scene who has access to defibrillator) 
...
rolling_count_resource_plot<-rolling_count_resource %>% 
  select(list_dates, order, contains("rollmean")) %>% 
  pivot_longer(contains("rollmean"), names_to="metric", values_to="val") %>% 
  mutate(name=fct_reorder(list_dates,order)) %>% 
  mutate (met_group=ifelse(str_detect(metric, "alloc"), "Mean allocated", "Mean arriving on scene")) %>% 
...
plot<-rolling_count_resource_plot %>%
  filter(type!="c1t") %>% 
...
## A&E attendances (rolling average)
...
roll_mean_ae_attend<-summattend %>% 
  mutate(meanattend_rollmean=rollmean(meanattend, k=12,fill=NA),
         meantype1_rollmean=rollmean(meantype1, k=12,fill=NA),
         meantype2_rollmean=rollmean(meantype2, k=12,fill=NA),
         meantypeoth_rollmean=rollmean(meantypeoth, k=12,fill=NA)) %>% 
  drop_na()
 

start_dates<-format(as.Date(seq(ymd('2018-04-01'),ymd('2021-04-01'),by='1 month')),"%Y-%m-%d")
end_dates<-format(as.Date(seq(ymd('2019-03-01'),ymd('2022-03-01'),by='1 month')),"%Y-%m-%d")

list_dates<-paste0(yearmonth(start_dates),"-",yearmonth(end_dates))
order<-c(1:37)

ae_attend_plots<-cbind(roll_mean_ae_attend,list_dates,order)

ae_attend_plots<-ae_attend_plots %>% 
  select(c(date, date2, list_dates, order, contains("rollmean")))%>% 
  pivot_longer(c(meanattend_rollmean:meantypeoth_rollmean), names_to = 'metric', values_to = 'counts') %>% 
  mutate(name=fct_reorder(list_dates,order)) %>% 
  mutate(met_lab=case_when(str_detect(metric, "meanattend")~ "Total attendances",
                           str_detect(metric, "type1")~ "Type 1 attendaces",
                           str_detect(metric, "type2")~ "Type 2 attendances",
                           str_detect(metric, "oth")~ "Total other attendances"))

plot<-ae_attend_plots %>% 
  ggplot(.,aes(x=name, y=counts, group=met_lab, colour=met_lab))+
  geom_line(linetype='solid')+
  scale_y_continuous(labels=comma)+
  theme_THF()+
  # facet_grid(cols=vars(met_cat))+
  scale_colour_THF()+
  labs(x = "", y="Number of attendances", caption = "NHS England, Ambulance Quality Indicators")+
  theme(legend.text=element_text(size=11),
        legend.title = element_blank(),
        axis.text.x=element_text(size=8, angle=90), 
        axis.text.y=element_text(size=11),
